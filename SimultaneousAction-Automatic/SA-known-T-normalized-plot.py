...
class Player:
    def __init__(self, T, N):
        ...
        #algorithm - it has been made 2-dimensional so that the weights can be plotted
        #print(self.weight)
        self.weight_normalized = np.zeros((self.N, self.T), dtype=float)
        self.epsilon = 10*math.sqrt(math.log(self.N) / self.T) #the value of learning parameter for the no regret case
        # under known T
    ...
    def pickStrategy(self, t): #this will be called for the t+1th time instance after the player has picked a strategy
        ...
        probability = self.weight[:, t] / weighted_total #get the probabilities for the individual weights to
        #randomize over
    ...
    def changeWeight(self, costs_decided, t, strategiesPicked, playerID, cost_scenarios): #given cost vector at time t generated by environment/adversary,
        # adding cost_scenarios because of the MW update rule even for those strategies that aren't picked but
        # need to be accounted for based on what the other player has picked
        ...
        for i in range(self.N):
            if (i == strategiesPicked[playerID]): #this will change for MWs, other strategies that were
                #not picked will have their weights change too depending on what the other players have
                # picked
            ...
            else: #now make use of the cost_scenarios list to account for the scenario under which the other
                #player strategy is picked in conjunction with each of the other strategies of this player
                #self.weight[i, t + 1] = self.weight[i, t]
                #basically for each i, we will find the appropriate index into costs_decided list using the
                #same sum method as we did in generateRewards()
                sum = 0
                sum += int(math.pow(self.N, playerID) * i) #get the appropriate sum contribution from this player
                #now, we have to add to sum the contributions from all the other players
                #iterate over the length of strategiesPicked list except for the playerID player
                for players in range(len(strategiesPicked)):
                    if (players == playerID):
                        continue #dont do anything if its the current player
                    else:
                        sum += int(math.pow(self.N, players) * strategiesPicked[players])
                #costs_decided is a 2D list indexed in with
                cost_now = cost_scenarios[sum][playerID] #this is the proper indexing in to get the cost
                #now, decrease the weights based on what cost you got from cost_now
                self.weight[i, t + 1] = self.weight[i, t] * math.pow((1 - self.epsilon), cost_now)
...
def play(players, playerID, strategiesPicked, players_strategies, t): #to play each game at each time step
    ...
    players_strategies[playerID].append(strategyPicked)

    #now, based upon the combination of picked strategies the same loss will be applied to each individual player

def changeWeights(players, playerID, strategiesPicked, costs_decided, t, cost_scenarios): #the function to change weights for each player
    # adding cost_scenarios because of the MW update rule even for those strategies that aren't picked but
    # need to be accounted for based on what the other player has picked
    players[playerID].changeWeight(costs_decided, t, strategiesPicked, playerID, cost_scenarios)
...
if __name__ == '__main__':
    ...
    for i in range(T): #at each time step, for each player, select an action and based on the scenario provided,
        ...
        for j in range(NumPl):
            t = threading.Thread(target=play, args=(players, j, strategiesPicked, players_strategies, i)) #first, pick a strategy
            t.start()
        ...
        for j in range(NumPl):
            t = threading.Thread(target=changeWeights, args=(players, j, strategiesPicked, costs_decided, i, cost_scenarios))
            #adding cost_scenarios because of the MW update rule even for those strategies that aren't picked but
            #need to be accounted for based on what the other player has picked
            t.start()
