...
class Environment: #the class that will be simulating the adversary in terms of generating rewards back and forth with
    ...
    #now, we have to literally calculate all of the possible permutations between other players and their strategies
        ...
        totalScenarios = (
                         self.numPlayers - 1) * self.N  # the different scenarios of strategies per remaining players that could
        # happen
        all_costs = []  # the list to store all the costs of each player referenced by player id
        if totalScenarios == 0:
            cost = input("Single Player game. Enter cost for picked strategy above: ")  # single player game
            all_costs.append(cost)  # append the cost decided for the single player setting
        else:
            for i in range(self.numPlayers):  # for each player, get the cost nd append to all_costs
                cost = input("Enter the cost for player %d: " % (i))  # prompt the user for cost
                all_costs.append(cost)
        logging.debug("Finished generating rewards for time %d" % (t))
        return all_costs  # return the cost for all players
