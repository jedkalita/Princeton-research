...
class Environment: #the class that will be simulating the adversary in terms of generating rewards back and forth with
    ...
    #now, we have to literally calculate all of the possible permutations between other players and their strategies
        ...
        try:
            ...
            totalScenarios = (
                             self.numPlayers - 1) * N  # the different scenarios of strategies per remaining players that could
            # happen
            expectedProfit = 0
            if totalScenarios == 0:
                ...
                for i in range(0,
                               totalScenarios):  # each scenario encoding a particular representation of opponents' strategies
                    profit = input(
                        "Enter the profit for scenario: %d" % (i + 1))  # prompt the user for cost against a config
...
class Player:
    ...
    def pickStrategy(self, t): #this will be called for the t+1th time instance after the player has picked a strategy
        ...
        eta = math.sqrt(math.log(self.N) * self.N / (2 * Tbar))  # the value of learning parameter between 0 and 1
        ...
        for i in range(len(probability)):
            ...
            y = (self.eta / self.N)
            print y'''
            probability[i] = ((1 - eta) * (self.weight[i] / weighted_total)) + (eta / self.N)
        # print(probability)
